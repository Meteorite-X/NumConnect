...
router.post('/auth/login', (req, res) => {
  ...
  if (!email.endsWith('@num.edu.mn')) return res.status(400).json({ error: '@num.edu.mn хаяг шаардлагатай' });
...
router.post('/auth/verify', (req, res) => {
  ...
  // Mock: 6 оронтой ямар ч код хүлээн авна (демо)
  if (!/^\d{6}$/.test(code)) return res.status(400).json({ error: 'OTP 6 оронтой байх ёстой' });
